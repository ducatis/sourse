...
window.addEventListener('DOMContentLoaded', () => {
  ...
  let modalState = {}; // (modalState => состояниеМодальногоОкна)Объект, куда помещаются все данные, которые выбрал пользователь в калькуляторе.
...
const checkNumInputs = selector => {
  ...
  numInputs.forEach(item => {
    // Перебираем псевдомассив, и каждому инпуту с воддом телефона добавляем коллбэк функцию. 
    item.addEventListener('input', () => {
      // Навешиваем оброботчик события 
...
const forms = () => {
  ...
  const message = {
    // Создаём переменную, в которую кладём объект с сообщениями.
  ...
  const clearInputs = () => {
    // Отдельно вынесенная функция по очищению инпутов.
    inputs.forEach(item => {
      // Берём переменную inputs, далее перебираем.
  ...
  form.forEach(item => {
    ...
    item.addEventListener('submit', e => {
      // Каждой форме навешиваем событие submit
      ...
      .then(res => {
        // Так как сдесь нам возвращается promis, то мы прописываем цепочку. То есть в этот then нам попадает какой-то результат который прислал нам сервер, причем в текстовом формате. Аргументом res (result) мы говорим, что к нам возвращается какой-то результат
      ...
      .finally(() => {
        // finaly выполниться в любом случае, не важно промис у нас ушел в положительную, либо в отрицательну.
        clearInputs(); // Вызываем отдельно вынесенную функцию по очищению инпутов.

        setTimeout(() => {
          // Создаём таймаут с функцией, что через 5 секунд удалим div с нашими сообщениями.
